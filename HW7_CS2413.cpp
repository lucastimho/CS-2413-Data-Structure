...
{
	return key % size; // Returns the mod of the key and size
}
void HashTable::CreateTable(int divisor)
{
	size = divisor;
	table = new Node *[size](); // Creates a Dynamic Allocated Table for nodes
}
Node *HashTable::Search(int key)
{
	int i = hash(key);
	Node *temp = table[i];
	while (temp != nullptr) // Loops through the table until the key is found
	{
		if (temp->Get_key() == key)
		{
			return temp;
...
{
	int i = hash(temp->Get_key());
	temp->Set_p(table[i]); // Adds the new node in front any nodes allocated for this space
	table[i] = temp;
}
void HashTable::Remove(int key)
{
	...
	Node *temp = table[i];
	Node *prev = nullptr;
	while (temp != nullptr) // Loops through Hash Table
	{
		if (temp->Get_key() == key) // Checks each key
		{
			if (prev == nullptr) // Checks if Node is at the beginning
			{
				table[i] = temp->Get_p();
			}
			else
			{
				prev->Set_p(temp->Get_p());
			}
			delete temp; // Removes node
			return;
		}
		prev = temp;
		temp = temp->Get_p();
...
{
	return size; // Returns size of the Hash Table
}
// --------------
// Main Function
// --------------
