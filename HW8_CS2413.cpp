...
// Node Node::Copy(Node *temp)
// {
// 	Node newNode;
// 	newNode.Set_key(temp->Get_key());
// 	newNode.Set_index(temp->Get_index());
// 	return newNode;
// }
...
int HashTable::hash(int key) // Returns hash key
{
	return key % size;
}

void HashTable::CreateTable(int divisor) // Creats table and initalizes all elements to Node constructor
{
	size = divisor;
	table = new Node[size];
}

int HashTable::Search(int key) // Searches HashTable based on key value
{
...
void HashTable::Add(Node temp) // Adds new Node to HashTable
{
	int tableSize = 0;
	for (int i = 0; i < size; ++i) // Loop to determine number of valid Node elements
	{
	...
	if (tableSize >= size) // Checks if HashTable is full
	{
		// Node *a;
		Node *newTable = new Node[2 * size];

		for (int i = 0; i < size; ++i) // Loop to Rehash each element for the new HashTable
		{
			int curr = i;
			while (curr != -1) // Loop to find valid indices coalesced chaining
			{
				int newIndex = table[curr].Get_key() % (2 * size);

				if (newTable[newIndex].Get_key() == -1)
				{
					// newTable[newIndex] = a->Copy(&table[curr]);
					newTable[newIndex].Set_key(table[curr].Get_key());
					newTable[newIndex].Set_index(table[curr].Get_key());
				}
				else
				{
					int newCurr = newIndex;
					while (newTable[newCurr].Get_index() != -1)
					{
						newCurr = newTable[newCurr].Get_index();
					}
					int newNextIndex = -1;
					for (int i = 0; i < size; ++i)
					{
					...
					newTable[newCurr].Set_index(newNextIndex);
					// newTable[newNextIndex] = a->Copy(&table[curr]);
					newTable[newNextIndex].Set_key(table[curr].Get_key());
					newTable[newNextIndex].Set_index(table[curr].Get_index());
				}
				curr = table[curr].Get_index();
			}
		}
		delete[] table;
		table = newTable;
	...
	{
		// table[index] = b->Copy(&temp);
		table[index].Set_key(temp.Get_key());
		table[index].Set_index(temp.Get_index());
	}
	else
	{
		int curr = index;
		while (table[curr].Get_index() != -1) // Loop to find index to set for the next index for skip list
		{
			curr = table[curr].Get_index();
		}
		int newIndex = -1;
		...
		table[curr].Set_index(newIndex);
		// table[newIndex] = b->Copy(&temp);
		table[newIndex].Set_key(temp.Get_key());
		table[newIndex].Set_index(temp.Get_index());
		size++;
	}
}

void HashTable::Remove(int key) // Removed Node given a key
{
	int i = hash(key);
	if (table[i].Get_key() == key)
	{
	...
	else
	{
		...
		while (curr != -1) // Loop through HashTable and connects indices connected by removed Node
		{
			if (table[curr].Get_key() == key)
			{
				table[prev].Set_index(table[curr].Get_index());
				table[curr].Set_key(-1);
				table[curr].Set_index(-1);
				return;
			}
			prev = curr;
			curr = table[curr].Get_index();
