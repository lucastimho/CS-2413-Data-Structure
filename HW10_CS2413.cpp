...
{
	if (s == "pre") // Performs pre-order traversal
	{
	...
	else if (s == "post") // Performs post-order traversal
	{
		if (p == NULL || p->Get_key() == 0)
		{
			return;
	...
	else if (s == "in") // Performs in-order traversal
	{
		if (p == NULL || p->Get_key() == 0)
		{
			return;
	...
	else if (s == "bf") // Performs breadth-first traversal
	{
		if (p == NULL || p->Get_key() == 0)
		{
			return;
		...
		while (front < rear)
		{
			...
			{
				queue[rear++] = curr->Get_left();
			}
			if (curr->Get_right() != NULL)
			{
				queue[rear++] = curr->Get_right();
			}
		}
	}
	else
...
//
int Height(Node *root) // Returns height of the Binary Tree
{
	if (root == NULL || root->Get_key() == 0)
	{
		return -1;
	}
	else
...
// as an example.)
//
Node *Convert2Array(Node *root, int height) // Converts list into an array given the root node
{
	if (root == NULL)
	{
		return NULL;
	...
	int front = 0, rear = 0;
	temp[rear].Set_key(root->Get_key());
	temp[rear].Set_left(root->Get_left());
	temp[rear].Set_parent(root->Get_parent());
	temp[rear].Set_right(root->Get_right());
	rear++;
	while (front < size)
	{
		if (temp[front].Get_key() != 0)
		{
			if (temp[front].Get_left() != NULL)
			{
				temp[rear].Set_key(temp[front].Get_left()->Get_key());
				temp[rear].Set_left(temp[front].Get_left()->Get_left());
				temp[rear].Set_parent(temp[front].Get_left()->Get_parent());
				temp[rear].Set_right(temp[front].Get_left()->Get_right());
			}
			if (temp[front].Get_right() != NULL)
			{
				temp[rear + 1].Set_key(temp[front].Get_right()->Get_key());
				temp[rear + 1].Set_left(temp[front].Get_right()->Get_left());
				temp[rear + 1].Set_parent(temp[front].Get_right()->Get_parent());
				temp[rear + 1].Set_right(temp[front].Get_right()->Get_right());
			}
		}
		front++;
		rear += 2;
