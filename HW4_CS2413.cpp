...
// Create function used to take an abritrary amount of input and create a linked list adding to the head as more inputs are added
void List::Create()
{
	int sid;
	float gpa;
...
// Lsize function that returns the size of the linked list
int List::Lsize()
{
	int count = 0;
	while (head != nullptr)
...
// Find function that takes the key as a parameter and returns the Node if the key in the Linked List
...
// Insert function that inserts a node at a given index. Returns 1 if successful and returns -1 if out of bounds
...
// Remove function used to remove a node. Returns 1 if successful and returns -1 if out of bounds
int List::Remove(int idx)
{
	...
	else
	{
		...
		if (curr != nullptr)
		{
			Node *temp = curr->Get_Pnext();
			curr->Set_Pnext(temp->Get_Pnext());
			delete temp;
			return 1;
		}
		else
...
// Reverse function that reverses the Linked List
void List::Reverse()
{
	Node *prev = nullptr;
	Node *curr = head;
...
// Clear function that clears the Linked List
void List::Clear()
{
	while (head != nullptr)
	{
